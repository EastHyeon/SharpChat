...
public class S_BroadcastExitGame : IPacket
{
	...
	public ArraySegment<byte> Write()
	{
		...
		count += sizeof(ushort);
		success &= BitConverter.TryWriteBytes(s.Slice(count, s.Length - count), (ushort)PacketID.S_BroadcastExitGame);
		count += sizeof(ushort);
		success &= BitConverter.TryWriteBytes(s.Slice(count, s.Length - count), this.playerId);
		count += sizeof(int);
		success &= BitConverter.TryWriteBytes(s, count);
...
public class C_ExitGame : IPacket
{
	...
	public ArraySegment<byte> Write()
	{
		...
		count += sizeof(ushort);
		success &= BitConverter.TryWriteBytes(s.Slice(count, s.Length - count), (ushort)PacketID.C_ExitGame);
		count += sizeof(ushort);
...
public class S_PlayerList : IPacket
{
	...
	public ArraySegment<byte> Write()
	{
		ArraySegment<byte> segment = SendBufferHelper.Open(4096 * 3);
		ushort count = 0;
		bool success = true;
